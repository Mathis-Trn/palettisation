...
import type { Simulation, TransportLoadResult, VehicleConfig } from "@/domain/types";
import { TRANSPORT_VEHICLE_PRESETS } from "@/domain/constants";
import { ApiError, computeTransportLoad } from "@/lib/api/client";
...
export function TransportPanel({ simulation }: { simulation: Simulation }) {
  ...
  async function handleCompute() {
    ...
    try {
      const loadResult = await computeTransportLoad(pallets, vehicle);
      setResult(loadResult);
    } catch (error) {
      setComputeError(
        error instanceof ApiError ? error.message : "Erreur inattendue lors du calcul de chargement."
      );
    } finally {
    ...
    <div className="flex flex-col gap-5">
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <div>
            <Button onClick={handleCompute} disabled={pallets.length === 0 || isComputing}>
              <Truck className="h-4 w-4" />
              {isComputing ? "Calcul en cours…" : "Calculer le chargement"}
            </Button>
            {pallets.length === 0 && <p className="mt-2 text-xs text-slate-500">Lancez d&apos;abord une optimisation dans l&apos;onglet Résultats.</p>}
            {computeError && <p className="mt-2 text-xs text-danger-500">{computeError}</p>}
          </div>
        </CardContent>
      </Card>
