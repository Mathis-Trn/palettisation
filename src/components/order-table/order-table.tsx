...
import { Copy, Download, Trash2, Upload, PlusCircle, AlertTriangle } from "lucide-react";
import { CSV_TEMPLATE, downloadTextFile, parseCartonsCsv } from "@/lib/import-export/csv";
import { CsvImportDialog } from "./csv-import-dialog";
import { isCartonOversizedForPallet } from "./oversized-check";
...
export function OrderTable({ simulation }: { simulation: Simulation }) {
  ...
  const [importPreview, setImportPreview] = useState<ReturnType<typeof parseCartonsCsv> | null>(null);
  ...
  function handleFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      ...
      setImportPreview(parseCartonsCsv(text));
    };
    reader.readAsText(file, "utf-8");
    e.target.value = "";
  }

  function confirmImport(mode: "append" | "replace") {
    if (!importPreview) return;
    const newRows: CartonLineRow[] = importPreview.validLines.map((line) => ({ ...line, id: uuid() }));
    if (mode === "replace") {
    ...
    <Card>
      <CardHeader className="flex-wrap gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="secondary" onClick={() => downloadTextFile("modele-cartons.csv", CSV_TEMPLATE, "text/csv;charset=utf-8")}>
...
function OrderRow({
  ...
  const oversized = isCartonOversizedForPallet(line, simulation.settings.palletConfig, simulation.settings.globalRotationsEnabled);
          ...
          {(errorMessage || oversized) && (
            <Tooltip>
              <TooltipTrigger asChild>
                <span>
                  <Badge variant={errorMessage ? "danger" : "warning"}>
                    <AlertTriangle className="h-3 w-3" />
                  </Badge>
                </span>
              </TooltipTrigger>
              <TooltipContent>{errorMessage ?? "Ce carton dépasse la palette dans toutes les orientations autorisées."}</TooltipContent>
            </Tooltip>
          )}
